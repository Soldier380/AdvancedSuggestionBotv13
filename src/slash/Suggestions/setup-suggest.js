...
let data = await GuildConfig.findOne({ guildId: interaction.guild.id })
  if(!data){
    const newdata = new GuildConfig({ guildId: interaction.guild.id, suggestionChannel: ch.id })
    newdata.save()
  } else {
    await GuildConfig.findOneAndUpdate({ guildId: interaction.guild.id }, { suggestionChannel: ch.id })
  }
